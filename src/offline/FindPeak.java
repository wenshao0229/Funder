...
public class FindPeak {
	...
	// Use LocalTime.isAfer/isBefore to compare to objects
	// TimeBucket: ["00:00", "00:15", "00:30", ..., "23:30", "23:45"]
	//currentTime: 10:10
	private static String findBucket(String currentTime) {
		LocalTime cur = LocalTime.parse(currentTime);
		int left = 0;
		int right = buckets.size() - 1;
		
		while (left < right - 1) {
			int mid = left + (right - left) / 2;
			if (buckets.get(mid).isAfter(cur)) {
				right = mid;
			} else {
				left = mid;
			}
		}
		return buckets.get(right).isAfter(cur) ? buckets.get(left).toString() : buckets.get(right).toString();
	}
}
